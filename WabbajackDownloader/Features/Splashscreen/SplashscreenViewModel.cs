...
internal sealed partial class SplashscreenViewModel : ObservableObject, INavigable
{
    [ObservableProperty]
    ...
    public SplashscreenViewModel(RepositoriesDownloader repoDownloader, UpdateHandler updateHandler, INavigator navigator)
    {
    ...
    async Task INavigable.OnNavigatedToAsync(object? parameter, CancellationToken cancellationToken)
    {
        ...
        await _updateHandler.CheckForUpdateAsync(cancellationToken);
        await _navigator.NavigateWindowAsync<MainWindow>(_updateHandler.IsOutdated, cancellationToken);
    }
}
